...
document.getElementById('chat-form').addEventListener('submit', async function (e) {
    ...
    const response = await fetch("https://router.huggingface.co/hf-inference/models/openai-community/gpt2", {
        method: 'POST',
        headers: { 
            'Authorization': 'Bearer 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', // 替换为你的 Hugging Face API 密钥
    ...
    if (data && data.answer) {
        ...
        // 将模型的回答添加到上下文
        conversationHistory += `Bot: ${data.answer}\n`;  // 记录模型的回答
    } else {
